...
# Dictionary to store active clients {username: (reader, writer, client_id)}
active_clients = {}
...
async def handle_client(reader, writer):
    ...
    if message.startswith("NAME:"):
        ...
        active_clients[username] = (reader, writer, client_id)
    else:
    ...
    async def send_file(filename, target_username):
        ...
        _, target_writer, _ = active_clients[target_username]
        
        if os.path.exists(filename):
            target_writer.write(f"FILE:{filename}".encode() + b'\n')
                ...
                if action == "1":  # List clients
                    ...
                    if action == "2":  # Send message
                        ...
                        for username in target_usernames:
                            if username in active_clients:
                                _, target_writer, _ = active_clients[username]
                                target_writer.write(f"MSG:{message}".encode() + b'\n')
                                await target_writer.drain()
                    ...
                    # Notify all clients
                    for name, (_, client_writer, _) in active_clients.items():
                        try:
    ...
    async def receiver():
        ...
        try:
            while True:
                data = await reader.readline()
                if not data:
                    username = next((name for name, (r, w, _) in active_clients.items() if r is reader), None)
                    if username:
